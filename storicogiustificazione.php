...
?>
<html>
<title>Storico Giustificazioni</title>
<meta charset="UTF-8">
...
<header class="w3-container w3-center testata" style="padding:40px 16px">
  <h2 class="w3-margin w3-jumbo testata">Storico Giustificazioni</h2>
</header>
...
if(isset($_POST["annulla"])){
    ...
    $query= "UPDATE giustificazione SET stato=1 WHERE idGiustificazione=$idrigainfo";  
    $result=mysqli_query($con,$query); 
    
    //$query_annulla= "SELECT * FROM permesso WHERE fkUtente=".$_SESSION['idutente']. " AND stato=0 AND idPermesso=".$idrigainfo;
    //echo $query;
    $query_annulla= "SELECT * FROM giustificazione WHERE idGiustificazione=".$idrigainfo;
...
//ELENCO DELLE GIUSTIFICAZIONI PER GENITORE 
$vetIdUtente= array();
$i= 0;
$query= "SELECT * FROM login WHERE nomeutente=".$_SESSION["nomeutente"]." AND password='".md5($_SESSION["password"])."'";
$result=mysqli_query($con,$query);
...
echo "<th>DATA</th><th>STUDENTE</th><th>CLASSE</th><th>TIPO</th>";
...
while($j < $i){
    ...
    //$query= "SELECT * FROM permesso WHERE fkUtente=".$_SESSION['idutente']. " AND stato=0 ORDER BY data";
    $query= "SELECT * FROM giustificazione WHERE fkUtente=".$vetIdUtente[$j]. " AND stato=0 ORDER BY data";
    $result=mysqli_query($con,$query);
        while($row= mysqli_fetch_array($result)){
        echo "<tr>";                              
        //echo "<td>";
       ...
       /* echo "<td>"; 
        echo "<input type='hidden' name='idrigaannulla' value='".$row["idGiustificazione"]."'>";
